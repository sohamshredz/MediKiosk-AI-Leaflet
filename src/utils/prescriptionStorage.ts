...
import { extractPrescriptionRules } from './prescriptionRuleExtractor';
...
export function loadPrescriptionsFromStorage(): PrescriptionRecord[] {
  ...
  try {
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      // Backward compatibility: older saved documents sometimes omitted the
      // medications array, which made the history card show zero medicines.
      return parsed.map((record: PrescriptionRecord) => {
        const existingMeds = Array.isArray(record.medications) ? record.medications : [];
        // Repair older records that were saved with an empty/missing medicine list.
        const repairedMeds = existingMeds.length > 0 || !record.ocrText
          ? existingMeds
          : extractPrescriptionRules(record.ocrText, record.patientName || '').medications
              .map(m => ({ ...m, prescriptionId: record.id }));
        return {
          ...record,
          medications: repairedMeds,
          recommendedTests: Array.isArray(record.recommendedTests) ? record.recommendedTests : [],

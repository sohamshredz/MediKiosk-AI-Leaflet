...
import { fetchPrescriptionsFromSupabase } from '../../utils/supabaseClient';
...
export const PrescriptionListView: React.FC<PrescriptionListViewProps> = ({
  ...
  const loadData = async () => {
    // Local cache makes the kiosk work offline; Supabase makes doctor-saved
    // e-prescriptions available to the patient later, including on another device.
    const localList = getPrescriptionsForPatient(currentPatient.id);
    setPrescriptions(localList);

    try {
      const remote = await fetchPrescriptionsFromSupabase(currentPatient.id);
      if (remote.prescriptions.length > 0) {
        const merged = [...remote.prescriptions, ...localList]
          .filter((rx, index, arr) => arr.findIndex(item => item.id === rx.id) === index)
          .sort((a, b) => new Date(b.updatedAt || b.createdAt).getTime() - new Date(a.updatedAt || a.createdAt).getTime());
        setPrescriptions(merged);
      }
    } catch (error) {
      console.warn('Unable to refresh prescriptions from cloud; showing local history.', error);
    }
  };
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-10 text-center space-y-4 shadow-xs transition-colors">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
              {searchQuery ? 'No prescriptions match your search query.' : 'No prescription records are available yet. Doctor e-prescriptions and scanned prescriptions will appear here automatically.'}
            </p>
          </div>
          <button
            type="button"

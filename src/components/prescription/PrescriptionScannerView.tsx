...
import { extractPrescriptionRules, sanitizeMedicationList, normalizeMedicineName, isKnownMedicineName, ExtractedPrescriptionData } from '../../utils/prescriptionRuleExtractor';
import { 
...
Dr. Ananya Sen, MD (Endocrinology)`;
  ...
  const validateAndStartScan = async (
    ...
    // 3. Real local OCR first. The previous version sent only a generic
    // placeholder string for uploaded images, so the rule extractor had no
    // medicine names to find. Tesseract gives us actual text even when Gemini
    // is unavailable or rate-limited.
    let baseOcrText = presetOcrText || '';
    if (!baseOcrText && !mimeType.includes('pdf')) {
      try {
        setProgressLabel('Reading prescription text locally (OCR)...');
        const worker = await createWorker('eng');
        const result = await worker.recognize(imageSrc);
        baseOcrText = result.data.text || '';
        await worker.terminate();
      } catch (ocrError) {
        console.warn('Local OCR failed; continuing with AI image scan:', ocrError);
      }
    }
    if (!baseOcrText) {
      baseOcrText = `Prescription document captured on ${new Date().toLocaleDateString()}.`;
    }
    ...
    try {
      ...
      if (data?.success && data?.extractedData) {
        ...
        if (localRuleResult.doctorName) {
          setDoctorName(localRuleResult.doctorName);
        } else if (ext.doctorName && ext.doctorName !== 'Not detected' && ext.doctorName !== 'Unclear') {
          setDoctorName(ext.doctorName);
        }

        if (localRuleResult.hospitalName) {
          setHospitalName(localRuleResult.hospitalName);
        } else if (ext.hospitalName && ext.hospitalName !== 'Not detected' && ext.hospitalName !== 'Unclear') {
        ...
        // Merge AI medicines with local OCR/rule medicines. Never replace a longer
        // local list with a shorter AI list: handwritten prescriptions commonly cause
        // multimodal AI to omit one or more rows.
        const aiMeds = sanitizeMedicationList(Array.isArray(ext.medications) ? ext.medications : []);
        const mergedMeds: PrescriptionMedication[] = localRuleResult.medications.map((localMed, idx) => {
          const localName = normalizeMedicineName(localMed.medicineName);
          const localKey = localName.toLowerCase().replace(/[^a-z0-9]+/g, '');
          const aiMed = aiMeds.find((candidate: any) => {
            const aiName = normalizeMedicineName(candidate.medicineName || '');
            return aiName.toLowerCase().replace(/[^a-z0-9]+/g, '') === localKey;
          }) || {};
          const medName = localName || normalizeMedicineName(aiMed.medicineName || '') || localMed.medicineName;
          const isLow = Boolean(aiMed.isLowConfidence) ||
            (typeof aiMed.confidenceScore === 'number' && aiMed.confidenceScore < 75) ||
            localMed.isLowConfidence || !medName;
          return {
            ...localMed,
            id: aiMed.id || localMed.id || `med-merged-${Date.now()}-${idx + 1}`,
            medicineName: medName,
            strength: localMed.strength || aiMed.strength || '',
            dosage: localMed.dosage || aiMed.dosage || 'Unclear',
            frequency: localMed.frequency || aiMed.frequency || 'As prescribed',
            duration: localMed.duration || aiMed.duration || 'As directed',
            route: localMed.route || aiMed.route || 'Oral',
            timing: localMed.timing || aiMed.timing || 'As directed',
            foodInstruction: localMed.foodInstruction || aiMed.foodInstruction || 'As directed',
            specialInstruction: localMed.specialInstruction || aiMed.specialInstruction || '',
            confidenceScore: Math.max(localMed.confidenceScore || 0, typeof aiMed.confidenceScore === 'number' ? aiMed.confidenceScore : 0),
            isLowConfidence: isLow,
            patientVerified: false
          };
        });
        // Only append AI-only medicines when their name resolves to the curated
        // medicine dictionary. This blocks hallucinated strings such as
        // "Cobimzbue" from replacing a locally recognized Cetirizine row.
        aiMeds.forEach((m: any, extraIdx: number) => {
          const canonical = normalizeMedicineName(m?.medicineName || '');
          const key = canonical.toLowerCase().replace(/[^a-z0-9]+/g, '');
          const exists = mergedMeds.some(existing => existing.medicineName.toLowerCase().replace(/[^a-z0-9]+/g, '') === key);
          const isCanonical = isKnownMedicineName(canonical);
          if (!canonical || exists || !isCanonical) return;
          mergedMeds.push({
            id: m.id || `med-ai-extra-${Date.now()}-${extraIdx + 1}`,
            medicineName: canonical,
            strength: m.strength || '', dosage: m.dosage || 'Unclear',
            frequency: m.frequency || 'As prescribed', duration: m.duration || 'As directed',
            route: m.route || 'Oral', timing: m.timing || 'As directed',
            foodInstruction: m.foodInstruction || 'As directed', specialInstruction: m.specialInstruction || '',
            confidenceScore: typeof m.confidenceScore === 'number' ? m.confidenceScore : 75,
            isLowConfidence: Boolean(m.isLowConfidence), patientVerified: false
          });
        });
        const cleanMergedMeds = sanitizeMedicationList(mergedMeds);
        if (cleanMergedMeds.length > 0) setMedications(cleanMergedMeds);

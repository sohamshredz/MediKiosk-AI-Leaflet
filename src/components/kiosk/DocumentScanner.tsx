...
export const DocumentScanner: React.FC<DocumentScannerProps> = ({
  ...
  const processImageWithGemini = async (imageSrc: string, fileName: string, docType: any) => {
    ...
    try {
      ...
      // Always run local OCR first for uploaded images. The document scanner
      // previously depended only on the server response, which could return
      // an empty diagnosis/medication section even when the image was readable.
      let localOcrText = '';
      if (!imageSrc.startsWith('http') && !imageSrc.startsWith('data:application/pdf')) {
        try {
          const worker = await createWorker('eng');
          const localResult = await worker.recognize(imageSrc);
          localOcrText = localResult.data.text || '';
      ...
      if (data.success && data.extractedData) {
        let extracted: any = data.extractedData || {};
        // Normalize the two backend schemas used by MediKiosk. If the API
        // omitted medicines/doctor details, recover them from local OCR.
        const localRules = localOcrText ? extractPrescriptionRules(localOcrText, '') : null;
        // Prefer deterministic OCR/rule medicines whenever available. AI can
        // hallucinate instruction text such as "After Food" as a medicine or
        // duplicate a row. The local parser is grounded in the actual OCR text.
        if (localRules?.medications?.length) {
          ...
          if (localMeds.length) {
            extracted.medications = sanitizeMedicationList(localMeds).map((m: any) => ({
              name: m.medicineName, dose: m.strength || m.dosage || '',
        ...
        const newDoc: ScannedDocument = {
          ...
          imageUrl: imageSrc,
          documentDate: extracted.documentDate || extracted.date || new Date().toISOString().split('T')[0],

...
import { CLINICAL_INTAKE_QUESTIONS, ClinicalQuestionDef } from '../../data/clinicalQuestions';
...
export const VoiceIntakeAssistant: React.FC<VoiceIntakeAssistantProps> = ({
  ...
  const activeLanguage = ((language || contextLanguage || 'en') as SupportedLanguage);
  ...
  const langConfig = SUPPORTED_LANGUAGES[activeLanguage] || SUPPORTED_LANGUAGES.en;
  ...
  // 1. Initialize Speech Recognition & Welcome Greeting
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recog.onresult = (event: any) => {
        ...
        if (transcript) {
          ...
          // Focus input smoothly without scrolling
          setTimeout(() => {
            if (inputRef.current) {
    ...
    // Set initial welcome greeting tailored to patient
    ...
    setCurrentQuestionStep('complaint');

    // Speak initial greeting asynchronously (non-blocking, only once)
  ...
  const handleSelectOption = (optionText: string, fromQuestionMsgId: string) => {
    ...
    if (currentQuestionStep === 'complaint') {
      ...
      if (!optionText.includes('None') && !optionText.includes('नहीं') && !optionText.includes('ಇಲ್ಲ') && !optionText.includes('இல்லை')) {
        ...
        if (optionText.toLowerCase().includes('breath') || optionText.includes('सांस') || optionText.includes('છાતી')) {
          setEmergencyAlert({
            detected: true,
            reason: 'Breathlessness / Acute Respiratory distress reported with primary symptoms.',
  ...
  const handleSendMessage = async (customText?: string) => {
    ...
    // Check if this was a review trigger (across all 10 supported languages)
    const lowerText = textToSend.toLowerCase();
    const isReviewTrigger = lowerText.includes('review') || 
      lowerText.includes('confirm') || 
      textToSend.includes('समीक्षा') || 
      textToSend.includes('पुष्टी') || 
      textToSend.includes('சரிபார்க்க') || 
      textToSend.includes('పరిశీలించండి') || 
      textToSend.includes('পর্যালোচনা') || 
      textToSend.includes('ચકાસો') || 
      textToSend.includes('ಪರಿಶೀಲಿಸಿ') || 
      textToSend.includes('പരിശോധിക്കുക') || 
      textToSend.includes('ਪੁਸ਼ਟੀ') ||
      currentQuestionStep === 'review';
    ...
    try {
      ...
      if (data.success) {
        ...
        const fallbackAck = questionBank.review?.question || t('intake.acknowledged', 'Thank you. Your health details have been safely noted.');
        const aiResponseText = data.replyInPatientLanguage || data.replyInEnglish || fallbackAck;
        ...
        const defaultReplies = [
          t('common.yes', 'Yes'),
          t('common.no', 'No'),
          t('intake.moreSevere', 'Symptoms Severe')
        ];
        const quickReplies = (data.suggestedQuickReplies && data.suggestedQuickReplies.length > 0)
  ...
  return (
    ...
    >
      ...
      {/* 2. Red Flag Emergency Alert Banner (shrink-0) */}
      {emergencyAlert.detected && (
        <div className="shrink-0 p-3.5 bg-rose-600 text-white flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-in fade-in shadow-md">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              type="button"
              onClick={() => alert('🚨 Emergency Alert Sent! On-duty triage nurse and medical emergency staff have been dispatched to this kiosk.')}
      ...
      {/* 3. Subtle Friendly Notice when AI is in fallback mode (Non-intrusive) */}
      {isAiOffline && (
      ...
      <div className="shrink-0 p-3.5 bg-white border-t border-slate-200">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            title={isListening ? t('voice.stop', 'Stop Recording') : t('voice.speak', 'Tap to Speak Voice Input')}
          >
            {isListening ? (
              <>
                <MicOff className="w-5 h-5" />
                <span className="text-xs font-bold hidden sm:inline">{t('voice.stop', 'Stop')}</span>
              </>
            ) : (
              <>
                <Mic className="w-5 h-5" />
                <span className="text-xs font-bold hidden sm:inline">{t('voice.speak', '🎤 Speak')}</span>
              </>
            )}
          </button>
          ...
          <div className="flex-1 relative">
            <input
              ...
              placeholder={
                isLoadingAi 
                  ? t('voice.analyzing', 'AI is analyzing your response...') 
                  : `${t('voice.typePlaceholder', 'Type symptoms or speak in')} ${langConfig.nativeName} (${langConfig.name})...`
              }
          ...
          {liveExtractedSymptoms.length > 0 && (
            ...
            >
              ...
              <span>{t('intake.reviewFinish', 'Review & Finish')}</span>
            </button>
          )}
        </div>
      </div>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-5 border border-slate-200 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h4 className="font-black text-slate-900 text-base">{t('intake.reviewTitle', 'Review Your Clinical Intake')}</h4>
                  <p className="text-xs text-slate-500">{t('intake.reviewSubtitle', 'Please confirm the details before sending to the doctor')}</p>
                </div>
              </div>
              <button
                type="button"
            ...
            {/* Symptoms List */}
            <div className="space-y-2">
              <h5 className="text-xs font-bold text-slate-500 uppercase">{t('intake.recordedSymptoms', 'Recorded Symptoms')}</h5>
              {liveExtractedSymptoms.length === 0 ? (
                <p className="text-xs text-slate-400 italic">{t('intake.noSymptomsYet', 'No symptoms recorded yet.')}</p>
              ) : (
                liveExtractedSymptoms.map((sym, idx) => (
                  <div key={idx} className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between text-xs sm:text-sm">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{t('common.duration', 'Duration')}: {sym.duration || t('common.notSpecified', 'Not specified')} • {t('common.severity', 'Severity')}: {sym.severity || 5}/10</p>
                    </div>
                    <span className="px-2 py-0.5 bg-teal-100 text-teal-800 rounded font-bold text-xs uppercase">
                      {sym.bodyPart || t('common.general', 'General')}
                    </span>
                  </div>
                ))
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs pt-1">
                {extractedPastHistory.length > 0 && (
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                    <span className="font-bold text-slate-700 block mb-1">{t('intake.pastConditions', 'Past Conditions:')}</span>
                ...
                {extractedMedications.length > 0 && (
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                    <span className="font-bold text-slate-700 block mb-1">{t('intake.currentMedications', 'Current Medications:')}</span>
                    <ul className="list-disc list-inside text-slate-600 space-y-0.5">
            ...
            {extractedAllergies.length > 0 && (
              <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-xs">
                <span className="font-bold text-amber-900 block mb-1">{t('intake.knownAllergies', 'Known Allergies:')}</span>
            ...
            {emergencyAlert.detected && (
              <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl flex items-center gap-2 text-xs text-rose-900 font-bold">
                <ShieldAlert className="w-4 h-4 text-rose-600 shrink-0" />
                <span>{t('intake.priorityEmergency', 'Priority Emergency Triage')}: {emergencyAlert.reason}</span>
              </div>
            )}
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-200">
              ...
              >
                {t('intake.continueIntake', 'Continue Intake')}
              </button>
              <button
                type="button"
              ...
              >
                {isSavingConsultation ? (
                  ...
                  <>
                    ...
                    <span>{t('intake.confirmSend', 'Confirm & Send to Doctor')}</span>
                  </>
                )}
              </button>
            </div>

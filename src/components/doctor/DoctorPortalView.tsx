...
import { PatientProfile, ScannedDocument, TimelineEvent, CareStream, HospitalStaffMember, PrescriptionRecord, PrescriptionMedication } from '../../types';
...
export const DoctorPortalView: React.FC<DoctorPortalViewProps> = ({
  ...
  const handleSaveAndSign = async () => {
    ...
    const prescriptionId = `rx-doc-${Date.now()}-${Math.floor(100 + Math.random() * 900)}`;
    const cleanPrescriptionItems = prescriptionItems.filter(p => p.medicineName.trim() !== '');
    const diagnosis = doctorDiagnosis || patient.clinicalSummary?.diagnosticHypothesesCDS?.[0]?.condition || 'Clinical assessment completed';

    // Convert the doctor's e-prescription form into the same persistent
    // PrescriptionRecord used by the Patient Portal and prescription history.
    const doctorMedications: PrescriptionMedication[] = cleanPrescriptionItems.map((item, index) => ({
      id: `med-${prescriptionId}-${index + 1}`,
      prescriptionId,
      medicineName: item.medicineName.trim(),
      strength: item.dosage?.trim() || 'Unclear',
      dosage: item.dosage?.trim() || 'As prescribed',
      frequency: item.timing?.trim() || 'As prescribed',
      duration: `${Math.max(1, Number(item.days) || 1)} days`,
      timing: item.timing?.trim() || 'As prescribed',
      confidenceScore: 100,
    ...
    const patientPrescription: PrescriptionRecord = {
      id: prescriptionId,
      ...
      hospitalName: 'Hospital OPD Clinic',
      prescriptionDate: new Date().toISOString().split('T')[0],
      sourceType: 'doctor_e_prescription',
      originalFileUrl: '',
      fileName: `E-Prescription-${prescriptionId}.json`,
      fileMimeType: 'application/json',
      medications: doctorMedications,
      diagnosis,
      recommendedTests: [],
      followUpDate: new Date(Date.now() + Math.max(1, Number(followUpDays) || 7) * 86400000).toISOString().split('T')[0],
      generalAdvice: doctorAdvice,
      verificationStatus: 'DOCTOR_REVIEWED',
      overallConfidence: 100,
      hasLowConfidenceFields: false,
      doctorReviewedBy: currentDoctorName,
      doctorReviewedAt: nowIso,
      doctorClinicalNotes: 'E-prescription digitally entered, signed and saved by the treating doctor.',
      auditLogs: [{
        id: `audit-${Date.now()}`,
        userId: currentDoctorId,
        userRole: 'DOCTOR',
        userName: currentDoctorName,
        action: 'DOCTOR_REVIEWED',
        resourceType: 'prescription',
        resourceId: prescriptionId,
        timestamp: nowIso,
        note: 'Doctor signed and saved e-prescription to the patient record.'
      }],
      createdAt: nowIso,
      updatedAt: nowIso
    };

    const saveResult = await saveOrUpdatePrescription(
      patientPrescription,
      { id: currentDoctorId, role: 'DOCTOR', name: currentDoctorName },
      'DOCTOR_REVIEWED',
      'Doctor signed and saved e-prescription; automatically added to patient prescription history.'
    );

    if (!saveResult.success) {
      alert(saveResult.error || 'Could not save the e-prescription. Please try again.');
      return;
    }

    // Keep the existing doctor-patient consultation state in sync as well.
    const updatedPatient: PatientProfile = {
      ...
      doctorNotes: {
        ...
        customDoctorDiagnosis: diagnosis,
        doctorPrescription: cleanPrescriptionItems,
        doctorAdvice,
